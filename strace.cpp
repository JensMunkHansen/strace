...
#ifndef HAVE_UTOA
static inline char *utoa(uintptr_t val, char* memory, int base = 10)
{
  char* res = memory;
  int i;
  res[xtoa_max_length - 1] = '\0';
...
static inline char *ptoa(const void *val, char* memory)
{
  ...
  char* buf = utoa(reinterpret_cast<uintptr_t>(val), memory + res_max_length, 16);
  char* result = memory;
  strcpy(result + 2, buf);
  result[0] = '0';
  result[1] = 'x';
  ...
  if (forkedPid != 0) {
    ...
    char* memory = memory_;
    {
      ...
      default:
        ...
        strcat(msg, itoa(sig, msg + msg_max_length));
        break;
      }
      if (m_bEnableColorOutput)
        strcat(msg, "\033[0m");
      ...
      // If a pthread
      strcat(msg, utoa(pthread_self(), msg + msg_max_length));
      ...
      strcat(msg, itoa(getppid(), msg + msg_max_length));
...
# error Only ARM, x86 and x86-64 are supported
    ...
    (__GNUC__ == 4 && (__GNUC_MINOR__ > 9 || \
      ...
      if (m_bAppendPID) {
        ...
        strcat(line, "(");
        strcat(line, itoa(getppid(), memory));
        strcat(line, ")");
        if (m_bEnableColorOutput)
          strcat(line, "\033[0m");
      }
